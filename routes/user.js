...
exports.getUserInfo = function(req,res){	
	...
	var getUserDetails = "SELECT p.firstName,p.lastName,p.registrationDate,s.avgReview,s.sellerId FROM ebay.person p  " +
		...
		if(err2){
			...
			getReviews(function(error,reviews){
				if(error){
					...
					var originalSellerId = user[0].sellerId;

					res.render("userdetails", {regDate:regDate, name : name , avgReview : avgReview , reviews : reviews.reviews ,originalSellerId : originalSellerId, sellerId : userId , reviewCount : reviewCount , latestReviews : latestFive});
				}
			},userId);	
...
function getReviews (callback,sellerId)
{
	...
	mysql.fetchData(function(err2,seller){
		if(err2){
			...
			"INNER JOIN" +
			" ebay.person p ON s.reviewedBy = p.userId where s.reviewedFor = '"+seller[0].sellerId+"' ORDER BY Date(s.modifiedDate) desc,Time(s.modifiedDate) desc";
...
exports.postSellerReview = function(req,res){
	...
	var sellerPersonId = req.body.sellerPersonId;
	...
	mysql.fetchData(function(error,results){
		...
		{				
			...
			if(results.length == 0)
			{
				...
				" ON o.orderId = p.orderId where p.orderedBy = '"+userId+"' and o.soldBy='"+sellerPersonId+"'";
